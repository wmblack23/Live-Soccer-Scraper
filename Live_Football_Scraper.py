...
while refresh == '': # Continuous loop to refresh the live scores
    ...
    for k in data:
        for i in k:
            if ":" in i and min_diff != '00':
                k[k.index(i)] = str(int(i[:i.index(":")]) - hour_diff) + ":" + str(abs(min_diff) + int(i[i.index(":") + 1:]))
            elif ":" in i and min_diff == '00':
                k[k.index(i)] = str(int(i[:i.index(":")]) - hour_diff) + i[i.index(":"):]
                
    for k in data:
        for i in k:
            if ":" in i and int(i[i.index(":") + 1:]) >= 60:
                k[k.index(i)] = str(int(i[:i.index(":")]) + 1) + ":" + str(int(i[i.index(":") + 1:]) - 60)
                
    for k in data:
        for i in k:
            if ":" in i and int(i[:i.index(":")]) >= 24:
                k[k.index(i)] = str(int(i[:i.index(":")]) -24) + "0:" + str(int(i[i.index(":") + 1:])) + " +1"
                
    for k in data:
        for i in k:
            if ":" in i and i[i.index(":") + 1:] == '0':
                k[k.index(i)] = i + '0'
